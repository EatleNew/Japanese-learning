# Japanese Learning

这是一个面向安卓手机的日语背单词 App，目标是背《标准日本语》初级上、初级下、中级上、中级下四本书的词汇。项目使用 Expo React Native 开发。

## 当前功能

- 按「初级 / 中级」「上册 / 下册」「课次」筛选单词。
- 浏览词库，并支持搜索日语、假名、中文和 romaji。
- 浏览词库支持按课次、假名、日语、中文、错题数排序。
- 背卡片：显示日语、假名、中文、来源课次和声调位置。
- 熟悉度标签：每个单词可标记为红色「不熟」、黄色「不太熟」、绿色「完全熟」。
- 标签可在浏览词库和背卡片界面直接修改，并会保存在本地进度中。
- 练习筛选：背卡片、选择题、输入测验和今日复习都可以只练指定熟悉度的词。
- 假名声调高亮：浏览和背卡片时，声调范围会在假名上用蓝色底线标出。
- 罗马音开关：浏览词库、背卡片、选择题、五十音图都可以显示或隐藏罗马音。
- 五十音图：包含清音、浊音、半浊音，显示平假名和片假名。
- 选择题：看到日语或只看假名，选择中文；干扰项优先来自同课、相邻课和相近释义。
- 输入测验：看到中文，输入日语汉字或假名。
- 手写题：看到中文，在手写区写日语汉字或假名；手写区支持横屏全屏，自查后按写对/写错计入复习。
- 答题后打标：选择题、输入测验、手写题出结果后，可以直接把当前词标为不熟、不太熟或完全熟。
- 自动打标：答对时默认把当前词标为绿色“完全熟”，答错时默认标为红色“不熟”，出结果后仍可手动改成其他标签。
- 测验范围：选择题、输入测验、手写题可以选择只考没练过的词，或考当前筛选下的全部词；进入测验时会随机生成本次题目顺序。
- 今日复习：根据答题结果做简单间隔复习。
- 本地保存进度：答对、答错、复习到期时间和熟悉度标签会保存在手机本地。
- 单词记录字段：每个词会记录答对次数、答错次数、下次复习时间、熟悉度标签和最后一次操作时间。
- 用户记录导入导出：可以把背诵记录导出为 JSON 文件，新版本安装后再导入恢复。
- 安卓顶部已做安全区适配，避免标题和系统状态栏重合。

## 词库

原始单词 JSON 已经放在项目内：

```text
assets/source-json
```

已导入 4936 个词：

- 初级上：1060 个
- 初级下：1063 个
- 中级上：1731 个
- 中级下：1082 个

导入脚本：

```bash
npm run import:vocabulary
```

脚本会读取 `assets/source-json/word*.json`，生成 App 使用的 `src/data/vocabulary.ts`。因此换一台电脑运行项目时，不需要再依赖外部下载目录。

## 本地运行

建议使用 Node.js `20.19.4` 或更高版本。

```bash
npm install
npm run start
```

然后在手机上安装 `Expo Go`，用手机扫描终端里的二维码即可试用。

## 得到安卓 APK

先登录 Expo：

```bash
npx eas-cli login
```

然后构建 APK：

```bash
npx eas-cli build -p android --profile preview
```

构建完成后，Expo 会给出 APK 下载链接。下载到手机后安装即可。
